...
STACK_NAME=my-site-example-com
...
stack-start:  ## Local Stack: Start Services
	...
	@echo "Now visit: http://my-site.localhost"
...
build-acceptance-servers: ## Build Acceptance Servers
	...
	@docker build backend -t iFlameing/my-site-backend:acceptance -f backend/Dockerfile.acceptance
	@echo "Build acceptance frontend"
	@docker build frontend -t iFlameing/my-site-frontend:acceptance -f frontend/Dockerfile

.PHONY: start-acceptance-servers
start-acceptance-servers: build-acceptance-servers ## Start Acceptance Servers
	@echo "Start acceptance backend"
	@docker run --rm -p 55001:55001 --name my-site-backend-acceptance -d iFlameing/my-site-backend:acceptance
	@echo "Start acceptance frontend"
	@docker run --rm -p 3000:3000 --name my-site-frontend-acceptance --link my-site-backend-acceptance:backend -e RAZZLE_API_PATH=http://localhost:55001/plone -e RAZZLE_INTERNAL_API_PATH=http://backend:55001/plone -d iFlameing/my-site-frontend:acceptance

.PHONY: stop-acceptance-servers
stop-acceptance-servers: ## Stop Acceptance Servers
	@echo "Stop acceptance containers"
	@docker stop my-site-frontend-acceptance
	@docker stop my-site-backend-acceptance
